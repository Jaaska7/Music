...
class MusicPlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!selectedSong.isNullOrEmpty()) {
            // Detén la reproducción actual y carga la nueva canción
            mediaPlayer?.stop()
            getSelectedMusic(selectedSong)
        } else {
            // Si no hay información, obtén la música normalmente
            counterPositon = getPosition()
            getMusic(getPosition())
        }
    }

    private fun getSelectedMusic(selectedSong: String) {
        // Obtiene la lista de música y encuentra la posición de la canción seleccionada
        viewModel.musicList.observe(this) {
            val position = it.indexOfFirst { musicModel -> musicModel.title == selectedSong }
            if (position != -1) {
                counterPositon = position
                getMusic(position)
            }
        }
    }
    ...
    private fun initText(musicModel: MusicModel){
        ...
        Glide.with(applicationContext)
            ...
            .apply(RequestOptions().placeholder(R.drawable.posterimagen).centerCrop())
            .error(R.drawable.posterimagen)
    ...
    private fun btnClick(listSize : Int){
        binding.btnPlayOrPause.setOnClickListener {
            if (counterIcon % 2 == 0){
                mPause()
                binding.btnPlayOrPause.setImageResource(R.drawable.play)
                counterIcon++
            }else{
                mStart()
                binding.btnPlayOrPause.setImageResource(R.drawable.pause)
                counterIcon++
            }
        }
